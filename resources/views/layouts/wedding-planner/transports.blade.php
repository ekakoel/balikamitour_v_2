    ...
    <div id="weddingTransport" class="col-md-12">
                    ...
                    <tr>
                        ...
                        <td class="pd-2-8 text-right">
                            <div class="table-action">
                                <a href="modal" data-toggle="modal" data-target="#update-wedding-planner-transport-{{ $wp_transport->id }}">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group ">
                                                    ...
                                                    <select name="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date') }}" required>
                                                        <option selected value="{{ dateFormat($wp_transport->date) }}">
                                                            {{ dateFormat($wp_transport->date) }}
                                                            {{ dateFormat($wedding_planner->checkin) == dateFormat($wp_transport->date) ? "Check-in" : "" }}
                                                            {{ dateFormat($wedding_planner->wedding_date) == dateFormat($wp_transport->date) ? "Wedding Ceremony" : "" }}
                                                            {{ dateFormat($wedding_planner->checkout) == dateFormat($wp_transport->date) ? "Check-out" : "" }}
                                                        </option>
                                                        @for ($wd = 0; $wd <= $wedding_planner->duration; $wd++)
                                                            @if (dateFormat('+'.$wd .' days',strtotime($wedding_planner->checkin)) != dateFormat($wp_transport->date))
                                                                <option value="{{ dateFormat('+'.$wd .' days',strtotime($wedding_planner->checkin)) }}">
                                                                    {{ dateFormat('+'.$wd .' days',strtotime($wedding_planner->checkin)) }} 
                                                                    {{ dateFormat($wedding_planner->checkin) == dateFormat('+'.$wd .' days',strtotime($wedding_planner->checkin)) ? "Check-in" : "" }}
                                                                    {{ dateFormat($wedding_planner->wedding_date) == dateFormat('+'.$wd .' days',strtotime($wedding_planner->checkin)) ? "Wedding Ceremony" : "" }}
                                                                    {{ dateFormat($wedding_planner->checkout) == dateFormat('+'.$wd .' days',strtotime($wedding_planner->checkin)) ? "Check-out" : "" }}
                                                                </option>
                                                            @endif
                                                        @endfor
                                                    </select>
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <textarea name="remark" class="tiny_mce form-control @error('remark') is-invalid @enderror" placeholder="Insert remark">{!! $wp_transport->remark !!}</textarea>
                                                    @error('remark')
                                                        <div class="alert alert-danger">{{ $message }}</div>
                                                    @enderror
                                                </div>
        ...
        <div class="row">
            <div class="col-md-12 text-right">
                <a href="modal" data-toggle="modal" data-target="#add-wedding-planner-transport-{{ $wedding_planner->id }}">
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group ">
                                                ...
                                                <select name="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date') }}" required>
                                                    ...
                                                    @for ($wd = 0; $wd <= $wedding_planner->duration; $wd++)
                                                        <option value="{{ dateFormat('+'.$wd .' days',strtotime($wedding_planner->checkin))) }}">{{ dateFormat('+'.$wd .' days',strtotime($wedding_planner->checkin))) }} 
                                                            {{ dateFormat($wedding_planner->checkin)) == dateFormat('+'.$wd .' days',strtotime($wedding_planner->checkin))) ? "Check-in" : "" }}
                                                            {{ dateFormat($wedding_planner->wedding_date)) == dateFormat('+'.$wd .' days',strtotime($wedding_planner->checkin))) ? "Wedding Ceremony" : "" }}
                                                            {{ dateFormat($wedding_planner->checkout)) == dateFormat('+'.$wd .' days',strtotime($wedding_planner->checkin))) ? "Check-out" : "" }}
                                                        </option>
