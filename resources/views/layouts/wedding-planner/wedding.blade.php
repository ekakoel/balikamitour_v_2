...
<div class="col-md-6">
    <div class="page-subtitle">
        ...
        @if ($wedding_planner->status == "Draft")
            <span>
                <a href="modal" data-toggle="modal" data-target="#update-wedding-detail-{{ $wedding_planner->id }}"> 
    ...
    <table class="table tb-list" >
        <tr>
            ...
            @if (dateFormat($wedding_planner->checkin) >= dateFormat($wedding_planner->checkout))
                <td class="htd-2 bg-error">
                    {{ dateTimeFormat($wedding_planner->checkin) }}
                </td>
            @else
                <td class="htd-2">
                    {{ dateTimeFormat($wedding_planner->checkin) }}
        ...
        <tr>
            ...
            @if ($wedding_planner->slot)
                @if (dateFormat($wedding_planner->checkin) < dateFormat($wedding_planner->wedding_date) and dateFormat($wedding_planner->checkout) > dateFormat($wedding_planner->wedding_date))
                    <td class="htd-2">
                        {{ dateFormat($wedding_planner->wedding_date)." ".date('(H.i)',strtotime($wedding_planner->slot)) }}
                    </td>
                @else
                    <td class="htd-2 bg-error">
                        {{ dateFormat($wedding_planner->wedding_date)." ".date('(H.i)',strtotime($wedding_planner->slot)) }}
        ...
        <tr>
            ...
            @if (dateFormat($wedding_planner->checkout) <= dateFormat($wedding_planner->checkin))
                <td class="htd-2 bg-error">
                    {{ dateTimeFormat($wedding_planner->checkout) }}
                </td>
            @else
                <td class="htd-2">
                    {{ dateTimeFormat($wedding_planner->checkout) }}
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" readonly name="checkin" class="form-control date-picker @error('checkin') is-invalid @enderror" value="{{ dateFormat($wedding_planner->checkin) }}" placeholder="@lang("messages.Select date")"  required>
                                    @error('checkin')
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" readonly name="checkout" class="form-control date-picker @error('checkout') is-invalid @enderror" value="{{ dateFormat($wedding_planner->checkout) }}" placeholder="@lang("messages.Select date")"  required>
                                    @error('checkout')
                                        <span class="invalid-feedback">
                                            {{ $message }}
                                        </span>
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" readonly name="wedding_date" class="form-control date-picker @error('wedding_date') is-invalid @enderror" value="{{ dateFormat($wedding_planner->wedding_date) }}" placeholder="@lang("messages.Select date")"  required>
                                    @error('wedding_date')
                                        <span class="invalid-feedback">
                                            {{ $message }}
                                        </span>
