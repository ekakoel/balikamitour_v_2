                ...
                @if ($receptionVenue)
                    @if ($orderWedding->service != "Reception Venue")
                        @if ($orderWedding->service != "Wedding Package")
                            <span>
                                <form action="/fdelete-reception-venue/{{ $orderWedding->id }}" method="post" enctype="multipart/form-data">
                                    @csrf
                                    @method('put')
        ...
        <div class="page-subtitle {{ $orderWedding->reception_venue_decoration_id?"":"empty-value"; }}">
            ...
            @if (count($reception_v_decorations)>0)
                @if ($orderWedding->reception_venue_decoration_id)
                    @if ($orderWedding->service != "Wedding Package")
                        <span>
                            <form action="/fdelete-decoration-reception-venue/{{ $orderWedding->id }}" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                <button class="icon-btn-remove" onclick="return confirm('Are you sure?');" type="submit" data-toggle="tooltip" data-placement="top" title="Remove"><i class="icon-copy fa fa-trash"></i></button>
                            </form>
