                        ...
                        <tr>
                            ...
                            <td class="pd-2-8 text-right">
                                <div class="table-action">
                                    <a href="modal" data-toggle="modal" data-target="#update-order-wedding-transport-{{ $wp_transport->id }}">
                            ...
                            @foreach ($transport_orders as $tr_no=>$transport_inv)
                                @if ($wp_transport->id == $transport_inv->transport_id)
                                    <tr>
                                        ...
                                        <td class="pd-2-8 text-right">
                                            <div class="table-action">
                                                <a href="modal" data-toggle="modal" data-target="#update-order-wedding-transport-{{ $transport_inv->id }}">
                                                    <button class="btn-edit" data-toggle="tooltip" data-placement="top" title="@lang('messages.Edit Transport')"><i class="icon-copy fa fa-pencil"></i></button>
                                                </a>
                                                        ...
                                                        <div class="row">
                                                                ...
                                                                <div class="form-group ">
                                                                    ...
                                                                    <select name="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date') }}" required>
                                                                        <option selected value="{{ dateFormat($transport_inv->date) }}">
                                                                            ...
                                                                            {{ dateFormat($orderWedding->checkout) == dateFormat($transport_inv->date) ? "Check-out" : "" }}
                                                                        </option>
                                                                        @for ($wd = 0; $wd <= $orderWedding->duration; $wd++)
                                                                            @if (dateFormat('+'.$wd .' days',strtotime($orderWedding->checkin)) != dateFormat($transport_inv->date))
                                                                                <option value="{{ dateFormat('+'.$wd .' days',strtotime($orderWedding->checkin)) }}">
                                                                                    {{ dateFormat('+'.$wd .' days',strtotime($orderWedding->checkin)) }} 
                                                            ...
                                                            <div class="col-md-12">
                                                                <div class="form-group">
                                                                    ...
                                                                    <textarea name="remark" class="ckeditor form-control @error('remark') is-invalid @enderror" placeholder="Insert remark">{!! $transport_inv->remark !!}</textarea>
                                                                    @error('remark')
                                                                        <div class="alert alert-danger">{{ $message }}</div>
                                                                    @enderror
                                                                </div>
