<div class="form-group">
    ...
    <select id="wedding_planner_id" class="form-control @error('wedding_planner_id') is-invalid @enderror" name="wedding_planner_id" required>
...
<div class="form-group">
    ...
    <select id="slot" class="form-control @error('slot') is-invalid @enderror" name="slot" required>
        <option selected value="">Select one</option>
        @for ($i = 0; $i < $duration; $i++)
            <option value="{{ $slot }}">{{ dateFormat('+1 days',strtotime($slot)) }}</option>
        @endfor
    </select>
</div>