        ...
        <div class="pd-ltr-20">
                            ...
                            @foreach ($promotions as $promotion)
                                <div class="bookingcode-card">
                                    ...
                                    <div class="content-card-promo">
                                        ...
                                        <button class="btn-remove-code m-t-10" data-toggle="tooltip" data-placement="top" title='@lang('messages.Ongoing promotion'){{" ". $promotion->name." "}}@lang('messages.and get discounts'){{ " $".$promotion->discounts." " }}@lang('messages.until'){{ " ". dateFormat($promotion->periode_end) }}'><i class="fa fa-question" aria-hidden="true"></i></button>
                                    </div>
                                </div>
                            @endforeach
                        @endif
            ...
            <div class="row">
                                ...
                                @if ($transport->cancellation_policy != "")
                                    <div class="pos-cancelation">
                                        <div class="btn-cancelation-policy" data-toggle="tooltip" data-placement="left" title="Cancelation Policy">
                                            <a href="modal" data-toggle="modal" data-target="#cancellation-policy-{{ $transport->id }}">
                                                        ...
                                                        @if (count($prices_daily_rent)>0)
                                                            <div class="tab-pane fade" id="daily-rent" role="tabpanel">
                                                                <table id="tbPriceDr" class="data-table table m-t-0" >
                                                                    ...
                                                                    <tbody>
                                                                        @foreach ($prices_daily_rent as $no=>$price_dr)
                                                                            @php
                                                                                $price_non_tax_dr = (ceil($price_dr->contract_rate / $usdrates->rate))+$price_dr->markup;
                                                                                $tax_dr = ceil(($taxes->tax/100) * $price_non_tax_dr);
                                                                                $normal_price_dr = $price_non_tax_dr + $tax_dr;

                                                                                if (isset($bookingcode->code) or isset($promotions)) {
                                                                                    if (isset($bookingcode->code)) {
                                                                                        $price_per_pax_dr = $normal_price_dr;
                                                                                        
                                                                                        if (isset($promotions)) {
                                                                                            $final_price_dr = $normal_price_dr - $bookingcode->discounts - $promotion_price;
                                                                                        }else{
                                                                                            $final_price_dr = $normal_price_dr - $bookingcode->discounts;
                                                                                        }
                                                                                    }else{
                                                                                        $price_per_pax_dr = $normal_price_dr ;
                                                                                        $final_price_dr = $normal_price_dr  - $promotion_price;
                                                                                    }
                                                                                }else {
                                                                                    $price_per_pax_dr = $normal_price_dr;
                                                                                    $final_price_dr = $normal_price_dr;
                                                                                }
                                                                            @endphp
                                                                            <tr>
                                                                                <td>
                                                        ...
                                                        @if (count($prices_airport_shuttle)>0)
                                                            <div class="tab-pane fade active show" id="airport-shuttle" role="tabpanel">
                                                                ...
                                                                <table id="tbPriceAs" class="data-table table m-t-0" >
                                                                    ...
                                                                    <tbody>
                                                                        @foreach ($prices_airport_shuttle as $no=>$price_as)
                                                                            @php
                                                                                $price_non_tax_as = (ceil($price_as->contract_rate / $usdrates->rate))+$price_as->markup;
                                                                                $tax_as = ceil(($taxes->tax/100) * $price_non_tax_as);
                                                                                $normal_price_as = $price_non_tax_as + $tax_as;

                                                                                if (isset($bookingcode->code) or isset($promotions)) {
                                                                                    if (isset($bookingcode->code)) {
                                                                                        $price_per_pax_as = $normal_price_as;
                                                                                        
                                                                                        if (isset($promotions)) {
                                                                                            $final_price_as = $normal_price_as - $bookingcode->discounts - $promotion_price;
                                                                                        }else{
                                                                                            $final_price_as = $normal_price_as - $bookingcode->discounts;
                                                                                        }
                                                                                    }else{
                                                                                        $price_per_pax_as = $normal_price_as ;
                                                                                        $final_price_as = $normal_price_as  - $promotion_price;
                                                                                    }
                                                                                }else {
                                                                                    $price_per_pax_as = $normal_price_as;
                                                                                    $final_price_as = $normal_price_as;
                                                                                }
                                                                            @endphp
                                                                            <tr>
                                                                                <td>
                                                        ...
                                                        @if (count($prices_transfers)>0)
                                                            <div class="tab-pane fade" id="Transfers" role="tabpanel">
                                                                ...
                                                                <table id="tbPriceTr" class="data-table table m-t-0" >
                                                                    ...
                                                                    <tbody>
                                                                        @foreach ($prices_transfers as $no=>$price_tr)
                                                                            @php
                                                                                $price_non_tax_tr = (ceil($price_tr->contract_rate / $usdrates->rate))+$price_tr->markup;
                                                                                $tax_tr = ceil(($taxes->tax/100) * $price_non_tax_tr);
                                                                                $normal_price_tr = $price_non_tax_tr + $tax_tr;

                                                                                if (isset($bookingcode->code) or isset($promotions)) {
                                                                                    if (isset($bookingcode->code)) {
                                                                                        $price_per_pax_tr = $normal_price_tr;
                                                                                        
                                                                                        if (isset($promotions)) {
                                                                                            $final_price_tr = $normal_price_tr - $bookingcode->discounts - $promotion_price;
                                                                                        }else{
                                                                                            $final_price_tr = $normal_price_tr - $bookingcode->discounts;
                                                                                        }
                                                                                    }else{
                                                                                        $price_per_pax_tr = $normal_price_tr ;
                                                                                        $final_price_tr = $normal_price_tr  - $promotion_price;
                                                                                    }
                                                                                }else {
                                                                                    $price_per_pax_tr = $normal_price_tr;
                                                                                    $final_price_tr = $normal_price_tr;
                                                                                }
                                                                            @endphp
                                                                            <tr>
                                                                                <td>
                ...
                <div class="col-md-12">
                    <div class="card-box m-b-18">
                        <div class="card-box-title">
                            <div class="subtitle"><i class="icon-copy fa fa-map-signs" aria-hidden="true"></i>@lang('messages.Similar to') {{ $transport->name }}</div>
                        </div>
                        
                        <div class="row m-l-m-8 m-r-m-8 ">
                            @foreach ($similartransports as $transports)
                                <div class="col-6 col-sm-4 col-md-4 col-lg-3 p-l-8 p-r-8">
                                    <div class="card">
                                        <div class="image-container-transport">
                                            <div class="first">
                                                <ul class="card-lable">
