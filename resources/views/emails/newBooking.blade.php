...
<body>
    ...
    This email was sent via the official Bali Kami Tour system on {{ dateTimeFormat($now) }}<br>
    To view order details, use the following link! <a href="{{ $order_link }}">See order</a><br> --}}
    ----------------------------------------------------------------------------------------------------------------------------<br>
    <h1 style="padding: 0 !important; margin: 0 !important;">{{ 'New Order '.$order->orderno }}</h1>
        ...
        <table class="table email-table">
            <tr>
                ...
                <td class="ftd-2">
                    ...
                    @elseif ($order->service == "Hotel Promo")
                        @php
                            $pn = json_decode($order->promo_name);
                            $promo_name = implode(", ",$pn);
                        @endphp
                        {{ $promo_name }}<br>
                        {{ $order->servicename }}<br>
                        {{ $order->subservice }}<br>
    ...
    @if (isset($airport_shuttles))
        ...
        ----------------------------------------------------------------------------------------------------------------------------<br>
            ...
            @foreach ($airport_shuttles as $no=>$airport_shuttle)
                <div class="text">{{ ++$no }}. Date {{ dateFormat($airport_shuttle->date) }}, Airport Shuttle, {{ $airport_shuttle->transport.", ".$airport_shuttle->src." - ".$airport_shuttle->dst  }} </div>
            @endforeach
        @endif
    @endif
