...
class OrdersAdminController extends Controller
{
    ...
    public function index()
    {
        ...
        $activeorders=Orders::with('reservations')
            ->where('status','active')
    ...
    //     // Bulk-load required relationships and data
    //     $rates = UsdRates::all();
    //     $usdrates = $rates->firstWhere('name', 'USD');
    //     $tax = Tax::find(1);
    ...
    //     $actionLogs = ActionLog::where('service_id', $id)
    //         ->where('service', 'Order')
    //         ->get();
    ...
    //     $totalAdditionalService = $additionalServiceData['services']
    //         ? collect($additionalServiceData['prices'])->zip($additionalServiceData['quantities'])->map(fn($pair) => $pair[0] * $pair[1])->sum()
    //         : 0;

    //     // Handle reservations
    //     if (!$order->rsv_id) {
    //         $reservationNo = $this->generateReservationNo($order, $now);
    //         $reservation = Reservation::create([
    //             'rsv_no' => $reservationNo,
    //             'checkin' => $order->checkin,
    //             'checkout' => $order->checkout,
    //             'agn_id' => $order->salesAgent->id,
    //             'adm_id' => Auth::id(),
    //             'status' => "Draft",
    //             'arrival_flight' => $order->arrival_flight,
    //             'arrival_time' => $order->arrival_time,
    ...
    //             ]);
    //         }
    //         $order->update(['rsv_id' => $reservation->id]);
    //     } else {
    //         $reservation = Reservation::find($order->rsv_id);
    //     }
    //     $guests = Guests::where('rsv_id', $order->rsv_id)->get();
    //     $invoice = InvoiceAdmin::where('rsv_id', $order->rsv_id)->first();
    //     $statusContract = $this->checkContractStatus($order, $reservation);
    //     $orderNotes = OrderNote::where('order_id', $id)->get();
    //     $receipts = $invoice ? PaymentConfirmation::where('inv_id', $invoice->id)->get() : null;
    //     $transports = Transports::where('status', "Active")->get();
    //     return view('admin.ordersadmindetail', compact(
    //         'now','order', 'rates', 'usdrates', 'tax', 'business', 'admins', 'orderlogs', 
    //         'attentions', 'guides', 'drivers', 'optionalRateOrder', 'optionalrates',
    //         'actionLogs', 'totalAdditionalService', 'reservation', 'guests', 
    //         'invoice', 'statusContract', 'orderNotes', 'receipts', 'transports'
    //     ));
    // }

    // private function generateReservationNo($order, $now)
    // {
    //     $agentCode = $order->salesAgent->code ?? 'AGT';
    //     $baseNo = $agentCode . $now->format('ymd');

    //     foreach (range('A', 'Z') as $suffix) {
    //         $reservationNo = $baseNo . $suffix;
    //         if (!Reservation::where('rsv_no', $reservationNo)->exists()) {
    //             return $reservationNo;
    //         }
    //     }

    //     return $baseNo . 'AA'; // Default fallback
    // }

    // private function checkContractStatus($order, $reservation)
    // {
    //     $invNo = "INV-" . $reservation->rsv_no;
    //     return File::exists("storage/document/invoice-{$invNo}-{$order->id}_en.pdf") ||
    //         File::exists("storage/document/invoice-{$invNo}-{$order->id}_zh.pdf");
    // }
...
// Function Delete Driver to Order =========================================================================================>
    ...
    // View Optional service add =========================================================================================>
    public function optional_rate_add($id){
        $order = Orders::where('id','=', $id)->first();
        ...
        if ($order->handled_by) {
            if ($order->handled_by == Auth::user()->id) {
                if ($order->status != "Approved") {
                    return view('admin.optional_service_add',compact('order'),[
                        'usdrates'=>$usdrates,
                        'order'=> $order,
                        'business'=>$business,
    ...
    public function func_update_optional_service_order(Request $request,$id){
        ...
        if (isset($order->extra_bed_price)) {
            $extra_bed_ud = json_decode($order->extra_bed_price);
            $extra_bed = array_sum($extra_bed_ud);
        }else{
            $extra_bed = 0;
        }
        if (isset($order->additional_service_qty)) {
        ...
        if (isset($order->promotion_disc)) {
            $pd_decode = json_decode($order->promotion_disc);
            $promotion_disc = array_sum($pd_decode);
        }else{
            $promotion_disc = 0;
        }

        $or_total_price = array_sum($price_total_nd);
        $optional_rate_price = json_encode($or_total_price);
        ...
        $price_total = json_encode($price_total_nd);
        
       
        

        $final_price = (($order->price_total + $or_total_price) - $order->discounts - $order->bookingcode_disc - $promotion_disc) + $total_Price_additional_service + $order->airport_shuttle_price ;
        // $final_price = ($optional_rate_price + $order->price_total)- $order->discounts - $order->bookingcode_disc;
        ...
        $order_log->save();
        //dd($order);
        return redirect("/orders-admin-$id")->with('success','Additional Charge has been updated');
    }
    ...
    public function fadd_optional_service_order(Request $request){
        ...
        $order_log->save();
        return redirect("/orders-admin-$request->order_id")->with('success','Optional service added successfully!');
    }
    ...
    public function func_edit_additional_services(Request $request,$id){
        ...
        if ($pd) {
            $promotion_disc = array_sum($pd);
        }else{
            $promotion_disc = 0;
        }
        
        $additional_services = json_encode($request->additional_service, true);
        $additional_service_dates = json_encode($request->additional_service_date, true);
        $additional_service_qtys = json_encode($request->additional_service_qty, true);
        $additional_service_prices = json_encode($request->additional_service_price, true);

        $adsers = json_decode($additional_services, true);
        $adser_dates = json_decode($additional_service_dates, true);
        $adser_qtys = json_decode($additional_service_qtys, true);
        $adsers_prices = json_decode($additional_service_prices, true);
        ...
        foreach ($adsers as $value_adser) {
            $new_adser[] = $value_adser;
        }
        foreach ($adser_dates as $value_adser_date) {
            $new_adser_date[] = $value_adser_date;
        }
        foreach ($adser_qtys as $value_adser_qty) {
            $new_adser_qty[] = $value_adser_qty;
        }
        foreach ($adsers_prices as $value_adser_price) {
            $new_adser_price[] = $value_adser_price;
        }
        ...
        if ($new_adser_price){
            $additional_service_total_price = [];
            $x = count($new_adser_price);
            for ($i=0; $i < $x; $i++) { 
                $price = $new_adser_price[$i] * $new_adser_qty[$i];
                array_push($additional_service_total_price, $price);
            }
            $total_additional_service = array_sum($additional_service_total_price);
        }else{
        ...
        $final_price = (($order->price_total + $order->optional_price) - $order->discounts - $order->bookingcode_disc - $promotion_disc) + $total_additional_service + $order->airport_shuttle_price;
        if (!$order->handled_by) {
            $handled_by = Auth::user()->id;
        }else{
            $handled_by = $order->handled_by;
...
// Function Updated Discounts =============================================================================================================>
    public function func_update_order_discounts(Request $request,$id){
        $order=Orders::findOrFail($id);
        $last_final_price = $order->final_price + $order->discounts;
        $discounts = $request->discounts;
        $final_price = $last_final_price - $discounts;
        $orderno =$order->orderno;
        if (!$order->handled_by) {
            $handled_by = Auth::user()->id;
        }else{
...
// ===============================================================================================================================================================
    ...
    public function fconfirmation_payment(Request $request,$id){
        ...
        $status = $request->status;
        if ($order->service !== "Wedding Package") {
            $payment = PaymentConfirmation::where('inv_id',$invoice->id)->first();
            if ($status == "Paid") {
                $order->update([
                    "status"=>"Paid",
                ]);
            ...
            $payment = PaymentConfirmation::where('id',$id)->first();
        }
